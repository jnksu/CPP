...
const  char * readFile[FLIGHT] = {					//读取文件
									"102.dat",
									"311.dat",
									"444.dat",
									"519.dat"
...
const  char * writeFile[FLIGHT] = {					//写入文件
									"102.dat",	
									"311.dat",
									"444.dat",
									"519.dat"
...
int readInformation(TICKET * , int , const char *);
int writeInformation(TICKET *, int , const char *);
void initializationSeats(TICKET *, int );
...
int main(int argc, char ** argv)
{
	...
	//提示用户
	while((flightNum = getRightFlight()) != -1)
	{
		fprintf(stdout, "You have use %s\n", *(readFile + flightNum));
		//初始化
		readInformation(inm, PLVOLUM, *(readFile + flightNum));
		//显示主菜单
		showMenu();
		//读取用户输入
		...
		//将用户修改重新写入文件
		writeInformation(inm, PLVOLUM, *(writeFile + flightNum));
	}
...
//获得正确的航班号,并返回对应的航班文件偏移
int getRightFlight(void)
{
	char choice;
	int offset;
	
	fputs("To choose a flight, entert its letter label:\n", stdout);
	fputs("a) 102	b)311\n", stdout);
	fputs("c) 444	d)519\n", stdout);
	fputs("q) Quit\n", stdout);
...
//从特定的文件读取信息
int readInformation(TICKET * box, int size,const char * readFile)
{	
	FILE * readIn;	
	int count;	//信息计数器
	TICKET temp;
	
	initializationSeats(box, PLVOLUM);
	//打开文件
...
//写入信息到指定的文件
int writeInformation(TICKET * source, int size,const char * writeFile)
{
	int count;	//信息计数器
	int index;
	FILE * writeOut;
...
//初始化座位
void initializationSeats(TICKET * tic, int size)
{
	int index;
	for(index = 0; index < size; index++){
		(tic + index)->seatid = index;
		(tic + index)->assigned = false;
	} 
}
//显示空位的数量
